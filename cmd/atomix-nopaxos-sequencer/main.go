...
import (
	...
	"github.com/atomix/atomix-nopaxos-sequencer/pkg/atomix/nopaxos"
...
func main() {
	...
	sequencerConfig := parseSequencerConfig()

	// Start the node. The node will be started in its own goroutine.
	node := atomix.NewNode(nodeID, partitionConfig, nopaxos.NewProtocol(sequencerConfig), registry.Registry)
...
func parseSequencerConfig() *nopaxos.SequencerConfig {
	protocolConfigFile := os.Args[3]
	protocolConfig := &nopaxos.SequencerConfig{}
